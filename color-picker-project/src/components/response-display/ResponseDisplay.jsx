...
const ResponseDisplay = ({ aiResponse, color }) => {
  ...
  return (
    <div className="response-container">
      {!aiResponse && (
        <p>
          Select a color, fill out the color details, and check back here for
      ...
      {aiResponse && (
        <h3 className="response-header">
          Your Personalized Color Recommendation:
        </h3>
      )}

      {aiResponse && formatResponseText(aiResponse)}

      {color && <ColorDisplay color={color} colorRecs={colorRecs} />}
    </div>
  );
};
